...
- For the `claude-bridge` provider (Claude Code's own OAuth session through a bridge extension, not a direct API key), usage comes from `globalThis[Symbol.for("pi.provider-usage.bus.v1")]`, the provider-usage-bus contract `@schuettc/pi-claude-bridge` (or any compatible bridge) publishes. gentle-shell never imports that package: it resolves the well-known symbol at runtime and validates every field defensively, exactly like the NaN payload below, so a mismatched or missing bus degrades to no data instead of throwing. Claude Code streams `rate_limit_event` on every turn, so a live subscription to the bus updates the bar and the panel immediately, without waiting for the 5-minute rule or `r`; a forced fetch (session start, provider switch, `r` in the panel) reads the bridge adapter's own `refresh()`, which answers from that same local state rather than making a new network request of its own. The `five_hour` and `seven_day` account windows become the main `claude` limit shown in the bar (`5h`) and the panel (`5h` and `week`); every other window the bus reports — a model family's 7-day share (`opus`, `sonnet`), OAuth apps, or a window a later bridge build adds — becomes its own row in the panel, named from the window's scope label or its id. When no bridge extension has published the bus at all, the panel says so directly (`claude-bridge · usage bus not published by this bridge build`) instead of the generic no-subscription-usage note. The bus can carry adapters for other usage providers, and any of them could claim `usageProvider: "claude"` too, so both the forced fetch and the live subscription pick the adapter whose `modelProviders` names `claude-bridge` — never the first `"claude"` match — and the subscription resolves that adapter once, then scopes every later event to its own `adapterId`, ignoring a snapshot published by any other adapter or one with no `adapterId` at all. Each snapshot's own `capturedAt` becomes its `fetchedAt`, so a forced refresh that resolves after a fresher live event cannot overwrite it: the store keeps whichever snapshot is newer for that provider and drops an older one that arrives late.
...
- Extensions can register a usage source for their own provider: gentle-shell has no built-in knowledge of it, but treats it exactly like Codex or NaN once registered. Emit `gentle-pi:usage-source/v1` on `pi.events` with `{ schema: "gentle-pi.usage-source/v1", provider, pendingNote?, fetch(apiKey, fetchFn, now) }`, where `fetch` resolves a `ProviderUsage` the same shape the built-in providers produce, or `undefined` when there is nothing to show yet. A malformed payload, a `fetch` that isn't a function, a provider id outside the safe id pattern, or a `fetch` call that throws or rejects is ignored rather than crashing the shell. Re-registering the same provider replaces its source, so emitting again at every `session_start` is safe and keeps load order irrelevant. Once registered, the provider shows `pendingNote` (or the same "no usage yet · r to fetch" default the built-ins use) until its first fetch, and a registration that arrives after the session already started, for the provider currently active, triggers one immediate refresh instead of waiting for the next turn or the 5-minute window. Example, using a neutral provider id:
  ...
  ```
- The bar names the subscription it shows (`codex`, `claude`, a NaN model) and always follows the active model. A provider with per-model allowances draws the session model's own meter, falling back to its family and then to the account total, never to whichever model the payload happens to list first — and that holds for a payload that reports a single metered model too, because one allowance is still per-model data rather than a reason to echo the first entry. The panel puts the active provider first, marked with the petal, and says why it has no data when it does not: API-key providers have no subscription windows, Claude and claude-bridge report after the first response or fetch, Codex and NaN wait for a fetch. A provider without per-model allowances keeps its single aggregate line in the bar, unchanged. Claude Pro/Max and `claude-bridge` both name their main limit `claude`, so the bar reads the same either way — only one of the two is ever the active provider in a given session, so this never shows two different things under one label.
