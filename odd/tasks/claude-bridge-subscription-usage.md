...
Two CodeRabbit findings on PR #1400 were confirmed against the code and fixed.

- Baseline before this round: focused 4-file run 195 tests, 194 pass, 1 known environmental failure.

**Finding 1 (major) — adapter selection.** `claudeBridgeAdapter` picked the first adapter with `usageProvider === "claude"`, so a foreign adapter claiming that same usage provider (but a different `modelProviders`) could be read instead of the bridge's own adapter, and `subscribeClaudeBridgeUsage` forwarded any live event carrying a `snapshot` regardless of which adapter published it.

- RED: added `fetchClaudeBridgeUsage selects the adapter by modelProviders, not the first usageProvider match` and `subscribeClaudeBridgeUsage scopes live events to the resolved bridge adapter id` to `tests/gentle-shell.test.ts` (plus a no-matching-adapter unsubscribe test), and added `adapterId: "schuettc.pi-claude-bridge"` to the file's `CLAUDE_BUS_SNAPSHOT` fixture. `node --experimental-strip-types --test --test-reporter=tap tests/shell-usage.test.ts tests/shell-usage-view.test.ts tests/shell-bar.test.ts tests/gentle-shell.test.ts` — 200 tests, 195 pass, 5 fail (the 1 known environmental failure + `fetchClaudeBridgeUsage selects...`, `subscribeClaudeBridgeUsage scopes live events...`, plus the two finding-2 RED tests below).
- Fix: `claudeBridgeAdapter` now matches on `modelProviders.includes("claude-bridge")` (plus `id`/`refresh` well-formedness) instead of `usageProvider === "claude"` alone; `subscribeClaudeBridgeUsage` resolves that adapter once at subscribe time and drops any event whose `snapshot.adapterId` does not equal the resolved adapter's `id` (including when `adapterId` is missing). Commit `42a6ca5e`.
- GREEN (this finding only, intermediate): the two new adapter tests passed; one assertion bug (`limits[0]?.usedPercent` instead of `limits[0]?.windows[0]?.usedPercent`) was caught and fixed by the same run before commit.

**Finding 2 (minor) — stale refresh overwrite.** A forced `refreshUsage` awaits `adapter.refresh()`; a live bus event can record a newer snapshot while that await is in flight, and `UsageStore.record` had no freshness check, so the slower forced-refresh result could silently overwrite the fresher live one.

- RED: added `parseProviderUsageBusSnapshot stamps fetchedAt from the snapshot's own capturedAt, falling back to now` to `tests/shell-usage.test.ts` (capturedAt preserved when present and valid; falls back to `now` when missing, zero, negative, `NaN`, or a non-number) and `UsageStore ignores a stale record and keeps equal or newer for the same provider` (older rejected, equal and newer both replace). Included in the same 200-test/5-fail RED run above.
- Fix: `parseProviderUsageBusSnapshot` now stamps `fetchedAt` from the snapshot's `capturedAt` (validated finite and `> 0`, falling back to `now` otherwise — confirmed against `@schuettc/pi-claude-bridge`'s `usage-bus.ts`, which stamps `capturedAt` with `Date.now()`, i.e. already epoch ms, so no unit conversion was needed) instead of always using `now`. `UsageStore.record` now drops an entry whose `fetchedAt` is strictly older than the already-stored entry for that provider; equal or newer still replaces, as before. Codex/Anthropic/NaN parsers were not touched — they always stamp `fetchedAt: now` at record time, so their forced refresh remains always-newer and is unaffected. Commit `2881fa8f`.
- GREEN (full round): `node --experimental-strip-types --test --test-reporter=tap tests/shell-usage.test.ts tests/shell-usage-view.test.ts tests/shell-bar.test.ts tests/gentle-shell.test.ts` — 200 tests, 199 pass, 1 fail (only the known pre-existing environmental failure). Net new: 5 passing tests (2 adapter-selection/scoping + 1 no-matching-adapter unsubscribe + 1 capturedAt-preservation + 1 UsageStore-freshness), baseline 195/194/1 → 200/199/1.
- Typecheck: `node scripts/check-types.mjs` reports the same 2 pre-existing diagnostics as the unmodified branch base (`extensions/gentle-ai.ts` TS2345 x2, `tests/sdd-native-managed-uptake.test.ts` TS2322 x1) — confirmed identical by running against `git stash` of this round's changes; 0 new diagnostics.
- `node scripts/build-runtime-modules.mjs --check`: runtime matches TypeScript sources (7 generated modules; one-shot metrics sources validated).
- Docs: `docs/gentle-shell.md`'s claude-bridge paragraph updated to describe adapter selection by `modelProviders`, live-event scoping by `adapterId`, and the `capturedAt`-based freshness guard.
- Commits: `42a6ca5e` (finding 1), `2881fa8f` (finding 2, includes the doc update).
- Not verified: the full unrestricted unit stage (`tests/*.test.ts` glob / `pnpm test`) per the task brief's known hang on this Windows host; only the 4-file focused run above was run locally, consistent with the prior closure's approach.
