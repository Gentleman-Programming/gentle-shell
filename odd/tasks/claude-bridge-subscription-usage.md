...
- Full unit stage (`node --experimental-strip-types --test tests/*.test.ts`) launched in the background at closure time; result to be recorded here once it returns.

## Next step

Record the full unit-stage result, confirm no new failures beyond the known baseline one, then close T4 (no PR until #1399 carries `status:approved`).
