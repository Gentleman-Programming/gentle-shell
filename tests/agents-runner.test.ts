...
test("childArguments builds an rpc launch with model, thinking, tools, session dir, and instructions", () => {
	...
	const isolated = childArguments(request({ model: undefined, thinking: undefined, extensions: ["/extensions/first.js", "", "/extensions/second.js"] }));
	assert.deepEqual(isolated.slice(isolated.indexOf("--no-extensions"), isolated.indexOf("--tools")), ["--no-extensions", "--extension", "/extensions/first.js", "--extension", "/extensions/second.js"]);
	assert.deepEqual(childArguments(request({ model: undefined, thinking: undefined, extensions: [] })).filter((arg) => arg === "--no-extensions" || arg === "--extension"), ["--no-extensions"]);
});
